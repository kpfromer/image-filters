...
export function LaplacianFilter(image: Pixel[][]) {
  ...
  for (let y = 0; y < image.length; y++) {
    for (let x = 0; x < image[y].length; x++) {
      ...
      const radiusArray = getRadiusArray(original, 1, x, y);
      // Get rgba values
